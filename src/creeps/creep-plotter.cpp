...
CreepPlotter::CreepPlotter(ALLEGRO_FONT* font, CreepLoader* creepLoader, std::list<Creep*>* creeps,
...
void CreepPlotter::plot(bool debug)
{
    ...
    if (debug)
    {
        for (int i = 0; i < creepCheckpoints->size() - 1; i++)
        {
            if (i < creepCheckpoints->size() - 1)
                al_draw_line(creepCheckpoints->at(i).first, creepCheckpoints->at(i).second, creepCheckpoints->at(i + 1).first,
                             creepCheckpoints->at(i + 1).second, al_map_rgb(0, 200, 200), 2);
    ...
    for (std::list<Creep*>::iterator it = this->creeps->begin(); it != this->creeps->end(); ++it)
    {
        ...
        auto frame =
            interpolateFrameToSprite(creep->walkingTimer, creep->totalWalkingTimer, spritesheet->walkingSprites[creep->direction].size());
        ...
        if (sprite != nullptr)
        {
            ...
            if (debug)
            {
                ...
                al_draw_text(this->font, al_map_rgb(255, 255, 255), creep->posX, creep->posY + 20, ALLEGRO_ALIGN_CENTER,
                             (std::to_string(frame) + "/" + std::to_string(spritesheet->walkingSpritesCount)).c_str());
            ...
            if (debug)
            {
                ...
                al_draw_line(creep->posX, creep->posY, creepCheckpoints->at(creep->cornerTarget).first,
                             creepCheckpoints->at(creep->cornerTarget).second, al_map_rgba(200, 0, 200, 127), 1);
            }
        }
    }
...
void CreepPlotter::drawHealthBar(Creep* creep, CreepSpritesheet* spritesheet)
{
    ...
    for (int i = 0; i < spritesheet->numberOfBars; i++)
    {
        ...
        if (alpha > 0)
            al_draw_tinted_bitmap(spritesheet->bars.second, al_map_rgba_f(alpha, alpha, alpha, alpha), creep->posX - barOffset + 4 * i,
                                  creep->posY - spritesheet->innerSpriteHeight, 0);
    }

    al_draw_text(this->font, al_map_rgb(255, 255, 255), creep->posX - barOffset - 10, creep->posY - spritesheet->innerSpriteHeight - 2,
                 ALLEGRO_ALIGN_CENTER, std::to_string((int)creep->health).c_str());
}
